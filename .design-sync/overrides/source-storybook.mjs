...
//
// Fork note: common.mjs and detect.mjs use the bundled-lib path so this
// override can sit in .design-sync/overrides/ while still pulling shared
// utilities from the real lib. story-imports.mjs resolves to the sibling
// override, which adds 'sb' to INERT_STUB so sb.mock() in preview.ts
// no longer crashes the decorator bundle at load time.
//
// esbuild is in .ds-sync/node_modules/ — the override dir has no node_modules
// so bare `import('esbuild')` would fail from here. Import via absolute URL.
const _esbuildUrl = new URL('../../.ds-sync/node_modules/esbuild/lib/main.js', import.meta.url);
async function _esbuild() { return import(_esbuildUrl.href); }
