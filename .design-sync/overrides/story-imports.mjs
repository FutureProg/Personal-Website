...
// Fork note: added "sb" to the explicit list so `import { sb } from
// "storybook/test"` resolves to the inert proxy instead of undefined.
// preview.ts calls sb.mock() at module scope for useGithubActivity mocking.
const INERT_STUB =
  ...
  'var m={};"fn action actions expect userEvent within waitFor screen fireEvent spyOn mocked jest vi configureActions decorateAction setupWorker http HttpResponse graphql rest sb".split(" ").forEach(function(k){m[k]=inert});' +
