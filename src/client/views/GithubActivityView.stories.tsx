        ...
        (Story, context) => {
            ...
            if (name === "Offline") {
                ...
                mocked(useGithubActivity).mockImplementation(() => ({
                    ...
                    connectionStatus: "error",
                    error:
                        "Error occurred connecting to the Activity Stream. \n Please check your internet connection",
                }));
            }
...
export const Online: Story = {
    ...
    play: async ({ canvasElement }) => {
        ...
        expect(canvas.getByText("FutureProg/repository-name"))
            .toBeInTheDocument();
    },
    render: (args, context) => {
        return React.createElement(() => {
            const [activities, setActivities] = useState(
                sampleFeed.slice(0, 1),
            );
            const addActivity = () => {
                let newItem =
                    sampleFeed[activities.length % sampleFeed.length]!;
                newItem = {
                    ...newItem,
                    repository: {
                        ...newItem.repository,
                        url: newItem.repository.url + "?t=" + Date.now(),
                    },
                }; // Ensure unique URL for view transition
                if (document.startViewTransition) {
                    document.startViewTransition(() => {
                        setActivities((prev) =>
                            [newItem!, ...prev].slice(0, sampleFeed.length)
                        );
                    });
                } else {
                    setActivities((prev) =>
...
export const Error: Story = {
    play: async ({ canvasElement }) => {
        const canvas = within(canvasElement);
        expect(canvas.getByText("Error occurred connecting to the Github API"))
            .toBeInTheDocument();
...
export const ConnectionFailure: Story = {
    play: async ({ canvasElement }) => {
        ...
        expect(
            canvas.getByText(
                "Error occurred connecting to the Activity Stream. Please check your internet connection",
            ),
        ).toBeInTheDocument();
        expect(canvas.getByText("ERROR")).toBeInTheDocument();
    },
